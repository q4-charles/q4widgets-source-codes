(function($) {
     ...
     * });
     */
    $.widget("q4.captcha", /** @lends q4.captcha */ {
        options: {
            /**
             ...
             * @type {interger}
             */
            submitTextFancy: 'Submit'
        },

        _create: function() {
            var _ = this, o = _.options;

            if (o.scrollOnConfirmation){
                _._scrollOnLoad();
            }
        },
        ...
        validateForm: function() {
            ...
            _.element.find('.RequiredField').each(function(){
                var element, $el = $(this).closest('tr'),
                    email = $el.find('input[name*="txtEmail"]'),
                    field = $el.find('td:first label').text();
            ...
            if (errors.length) {
                _.element.find('.ErrorContainer').html( Mustache.render( _.options.errorsTpl, errors ) ).fadeIn();
                return false;
            } else {
        ...
        _scrollOnLoad: function() {
            ...
            if ( _.element.find('.EditSubscriberConfirmation').text().trim().length > 0 ) {
                $('html, body').animate({
                    scrollTop: ( $('.EditSubscriberConfirmation').filter(':visible').offset().top - o.scrollOffset )
                }, 1000);
        ...
        _onEnterPress: function() {
            ...
            _.element.find('input[type="text"]').on('keydown', function(e){
                if (e.keyCode == 13) {
                    e.preventDefault();
                    if ( _.validateForm() ) {
        ...
        _beforeSubmit: function() {
            ...
            $captcha.find('input[type="submit"]').val( _.options.submitTextFancy );
